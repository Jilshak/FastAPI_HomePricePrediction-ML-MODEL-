...
@price.post("/predict/")
def predict_price(area_type: str, sqft: float, bath: int, balcony: int, bhk: int):
    print(data_columns)
    if area_type in data_columns["data_columns"]:
        loc_index = data_columns["data_columns"].index(area_type)
